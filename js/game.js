class Game {
    constructor() {

    }
    getState() {
        var gameStateRef = database.ref('gameState');
        gameStateRef.on("value", function (data) {
            gameState = data.val();
        })

    }

    update(state) {
        database.ref('/').update({
            gameState: state
        });
    }
    async start() {
        if (gameState === 0) {
            player = new Player();
            var playerCountRef = await database.ref('playerCount').once("value");
            if (playerCountRef.exists()) {
                playerCount = playerCountRef.val();
                player.getCount();
            }
            form = new Form()
            form.display();
        }
        player1 = createSprite(200, 20);
        player1.addImage("player1", player_img);

        player2 = createSprite(800, 20);
        player2.addImage("player2", player_img);
        players = [player1, player2];

    }

    play() {
background(back_img)

        form.hide();
      
      database.ref('player1').on('value',(data) => {  
         if (player1 = 'player'+player.index){
             stroke('green')
             fill('yellow')

         }else{
            stroke('yellow')
            fill('yellow')
         }
           
                               })   
                     
        Player.getPlayerInfo();
            
        var x = 100;
        var y = 20;
        var index = 0;
     
        for (var plr in allPlayers) {

           
            index = index + 1;
            x = 500 - allPlayers[plr].distance;
       
console.log(  players[index - 1].y)
            players[index - 1].x = x;
            players[index - 1].y = y;
   
            database.ref('player'+player.index).on('value',(data) => {  
         
                stroke('yellow')
                fill('yellow')
                
                text(`${allPlayers[plr].name}`,x,y+60)
                        })   
   
}
    


        drawSprites();  





        if (frameCount % 20 === 0) {
            fruits = createSprite(random(100, 1000), 0, 100, 100);
            fruits.velocityY = 6;
            var rand = Math.round(random(1, 5));
            switch (rand) {
                case 1:
                    fruits.addImage("fruit1", fruit1_img);
                    break;
                case 2:
                    fruits.addImage("fruit1", fruit2_img);
                    break;
                case 3:
                    fruits.addImage("fruit1", fruit3_img);
                    break;
                case 4:
                    fruits.addImage("fruit1", fruit4_img);
                    break;
                case 5:
                    fruits.addImage("fruit1", fruit5_img);
                    break;
            }
            fruitGroup.add(fruits);
            

        }

        if (player.index !== null) {
            for (var i = 0; i < fruitGroup.length; i++) {
                if (fruitGroup.get(i).isTouching(players)) {
                    fruitGroup.get(i).destroy();
score++;
                    database.ref('player'+player.index).update({
                        score: score
                    });
                }

            }
        }
    
    }
    end() {
        console.log("Game Ended");
    }
}
